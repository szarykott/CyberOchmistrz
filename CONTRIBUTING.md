...
```mermaid
flowchart LR
    PR["Pull Request"] -->|trigger| CI["ci.yml"]
    CI --> install["npm install"]
    install --> test["npm test"]
    test --> build["npm run build"]

    Push["Push to master"] -->|trigger| CD["cd.yml"]
    CD --> install2["npm install"]
    install2 --> patchBase["sed basePath = /CyberOchmistrz"]
    patchBase --> patchManifest["sed manifest URLs"]
    patchManifest --> build2["npm run build"]
    build2 --> nojekyll["touch out/.nojekyll"]
    nojekyll --> deploy["Deploy out/ to gh-pages branch"]
```
